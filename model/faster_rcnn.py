...
class FasterRCNN(nn.Module):
    ...
    def __init__(self, extractor, rpn, head,
        ...
        self.extractor = extractor#用于提取feature map的模型
        self.rpn = rpn#region proposal network
        self.head = head# rcnn 分类+回归
    ...
    def forward(self, x, scale=1.):
        ...
        * :math:`L` is the number of classes excluding the background.
        ...
        """
        img_size = x.shape[2:] # 0 1 是batch 和channel  img_size 宽和高

        h = self.extractor(x)#获得模型特征层的feature map
        #RPN 定位损失和得分 ，rois 和rois的索引
    ...
    def _suppress(self, raw_cls_bbox, raw_prob):
        ...
        for l in range(1, self.n_class):
            ...
            prob_l = prob_l[mask]
            keep = nms(cls_bbox_l.cuda(), prob_l.cuda(),self.nms_thresh)
    ...
    def predict(self, imgs,sizes=None,visualize=False):
        ...
        for img, size in zip(prepared_imgs, sizes):
            ...
            scale = img.shape[3] / size[1]#计算尺度
            #得到roi roi相对于bbox的locs 和得分
            ...
            # roi 变成了 接近 bbox的 预测框predict box
            cls_bbox = loc2bbox(at.tonumpy(roi).reshape((-1, 4)),
                                at.tonumpy(roi_cls_loc).reshape((-1, 4)))
            cls_bbox = at.totensor(cls_bbox)
            cls_bbox = cls_bbox.view(-1, self.n_class * 4)
            # clip bounding box 限制在原图内
